...
class LabelGUI(tk.Tk):
    def __init__(self, blobio: AzureBlobIO, paths: Paths, split_ratio: float, yolo_thr: float, cnext_thr: float, tolerance: float):
        ...
        # NEW: dict box_blob -> label (so we can browse and see status fast)
    ...
    def _build_ui(self):
        ...
        # Keep hotkeys for labeling (as before)
        ...
        # Optional: hotkeys for navigation that don't conflict
    ...
    def load_camera(self):
        ...
        # NEW: load all labels for this camera (box_blob -> label)
        self.labels_map = get_labels_for_cam(self.con, cam)

        # resume to first unlabeled (but user can browse freely after)
        self.ptr = 0
        while self.ptr < len(self.df) and str(self.df.iloc[self.ptr]["box_blob"]) in self.labels_map:
            self.ptr += 1
        if self.ptr >= len(self.df):
            self.ptr = 0  # allow browsing even if everything labeled
    ...
    def show_current(self):
        ...
        # Prefetch next row (for browsing)
    ...
    def on_label(self, label: str):
        ...
        # If already labeled, we don't allow re-label (to avoid dataset inconsistencies)
        ...
        # store label in sqlite + update in-memory map
        ...
        # Exports per rules with tolerance
        if label == "TP":
        ...
        # After labeling, jump to next unlabeled (fast workflow),
        # but user can still browse with <= / =>
        nxt = self._advance_to_next_unlabeled_from(self.ptr + 1)
        if nxt is None:
            # stay where you are, but show status updated
            self.show_current()
