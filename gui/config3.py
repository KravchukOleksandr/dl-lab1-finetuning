...
def load_config(config_path: str | None = None) -> AppConfig:
    ...
    max_per_camera = int(sampling.get("max_per_camera", 0) or 0)
    sampling_mode = str(sampling.get("mode", "uniform_time")).strip().lower()
    sampling_seed = int(sampling.get("seed", 123))

    if sampling_mode not in ("first_k", "uniform_time"):
