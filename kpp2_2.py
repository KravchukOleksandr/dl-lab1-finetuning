...
# ------------------------------------------------------------
# Логика:
# - если в MOTION_ZONE есть движение -> включаем YOLO
# - дальше несколько processed-кадров держим YOLO активным
# - если нижняя центральная точка bbox фуры ВПЕРВЫЕ вошла
#   в TRIGGER_ZONE -> сразу сохраняем 1 кадр
#
# Без argparse. Все параметры сверху.
# ============================================================
...
# Если OpenCV врёт про fps, задай руками. Иначе None.
...
FRAME_STEP = 10   # при 60 fps это ~6 fps
...
# Маленькая зона, куда должна ВОЙТИ нижняя центральная точка bbox
...
# Зона для детекции движения.
# Должна быть БОЛЬШЕ и ЧУТЬ РАНЬШЕ trigger-zone по траектории.
MOTION_ZONE_POLYGON = np.array([
    [650, 520],
    [1160, 520],
    [1160, 900],
    [650, 900],
...
# На сколько уменьшать кадр перед motion detection
MOTION_DOWNSCALE = 0.5

# Порог бинаризации разницы кадров
MOTION_DIFF_THRESHOLD = 18

# Минимальная доля "изменившихся" пикселей в motion-zone,
# чтобы считать, что есть движение
MOTION_ACTIVE_RATIO = 0.0035

# После обнаружения движения держим YOLO активным ещё N processed-кадров
# Это важно, чтобы не пропустить медленно входящую фуру
YOLO_HOLD_STEPS_AFTER_MOTION = 8

# Первые N processed-кадров всегда гоняем YOLO, чтобы "прогреться"
WARMUP_STEPS = 8

# Небольшой blur перед сравнением кадров
...
def scale_polygon(poly, scale):
    return np.round(poly.astype(np.float32) * scale).astype(np.int32)
...
def main():
    ...
    # Motion masks in downscaled space
    cap_h = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    cap_w = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    motion_h = int(round(cap_h * MOTION_DOWNSCALE))
    motion_w = int(round(cap_w * MOTION_DOWNSCALE))

    motion_zone_scaled = scale_polygon(MOTION_ZONE_POLYGON, MOTION_DOWNSCALE)
    ...
    while frame_idx < frame_count:
        ...
        if prev_motion_gray is None:
            ...
            motion_is_active = motion_r >= MOTION_ACTIVE_RATIO
        ...
        if not run_yolo:
            # Если YOLO не запускали, считаем что в trigger-zone нового входа не было.
        ...
        # Rearm: ждём, пока trigger-zone освободится
        if not current_in_trigger:
            if not armed:
                empty_steps += 1
                if empty_steps >= REARM_EMPTY_STEPS:
