...
def main() -> None:
    ...
    config.update(
        {
            ...
            "samples_per_cell_per_batch": train_sampler.per_cell,
            "train_cell_counts": {
    ...
    print(
        f"Each train batch: {len(train_sampler.groups)} cells x "
        f"{train_sampler.per_cell} samples = {args.batch_size}"
    )
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if args.selection_metric == "overall_fpr_at_recall_95":
            selection_value = val_metrics["overall"]["fpr_at_recall_95"]
        else:
