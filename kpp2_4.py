# ============================================================
# KPP2 "SCALES" FAST SNAPSHOT EXTRACTOR
# ------------------------------------------------------------
# Ускоренный вариант:
# - последовательное чтение cap.read()
# - motion gating только в enlarged trigger-zone
# - YOLO на GPU
# - снимок в момент первого входа bottom-center в trigger zone
# ============================================================
...
# GPU
DEVICE = 0              # 0 = первая CUDA карта
USE_HALF = True         # fp16 на GPU
...
# Движение меряем в слегка увеличенной trigger-zone
...
# Если несколько truck — выбираем лучший относительно этой точки
...
def main():
    ...
    print(f"Effective sampled FPS: {fps / FRAME_STEP:.2f}")
    ...
    motion_zone_scaled = np.round(motion_zone_polygon * MOTION_DOWNSCALE).astype(np.int32)
    ...
    while True:
        ...
        # пропускаем большинство кадров БЕЗ seek
        ...
        if not run_yolo:
            ...
            if processed % PRINT_EVERY_PROCESSED == 0:
                elapsed = time.time() - t0
                print(
                    f"processed={processed} time={time_sec/60:.1f} min "
                    f"captures={len(records)} armed={armed} run_yolo={run_yolo} "
                    f"yolo_runs_total={yolo_runs} motion_hits={motion_hits} "
                    f"hold={yolo_hold_steps} motion_ratio={motion_r:.5f} "
        ...
        if truck is not None:
            ...
            # сохраняем в самый ранний момент входа
    ...
    print(f"Wall time: {total_elapsed/60:.2f} min")
