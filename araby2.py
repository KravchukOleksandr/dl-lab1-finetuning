...
# Карта индексов классов -> имена (ваши бизнес-названия)
CLASS_NAMES = {0: "male", 1: "female", 2: "staff", 3: "car"}

# Порог детекции/IoU YOLO
CONF_THRESH = 0.25
IOU_THRESH = 0.45
IMG_SIZE = 1280                                # размер инференса (как при обучении)

# Трекинг
TRACKER_CONFIG = "bytetrack.yaml"              # встроенный ByteTrack

# Подтверждения (устойчивость к шуму)
INIT_CONSEC_FRAMES = 3     # подряд кадров, чтобы активировать трек
ENTRY_CONFIRM_FRAMES = 3   # подтверждение входа в зону
EXIT_CONFIRM_FRAMES = 5    # подтверждение выхода из зоны
LOST_FRAMES_TOLERANCE = 10 # сколько кадров терпим пропажу до закрытия трека

# Фильтр «микро-входов» (НОВОЕ)
MIN_ZONE_DWELL_SEC = 1.0   # не логировать вход/выход, если в зоне были < 1 сек (для классов 0/1)

# Поведение по классам
ENABLE_CAR_LOGGING = True  # писать ли события для class 3 (car)
PERSON_CLASSES = {0, 1}    # считаем зоны только для male/female
...
def main():
    # --- Подготовка видео ---
    cap_info = cv2.VideoCapture(SOURCE_VIDEO)
    ...
    if Path(ZONES_JSON_PATH).exists():
        ...
        print(f"[INFO] Зоны: загружено {len(zones_px)} ключей из {ZONES_JSON_PATH}")
    else:
    ...
    #   'entry_time' (str|None),
    #   'entry_frame' (int|None),   # кадр входа для фильтра <1с
    #   'entry_confirm', 'exit_confirm'
    # }
    #
    # для CAR (3):
    #   те же базовые поля, но считаем только вход/выход в кадр (zone="None")
    track_state = {}

    frame_idx = -1

    # Потоковый трекинг: результат-генератор по кадрам
    tracker = model.track(
        source=SOURCE_VIDEO,
        stream=True,
        ...
        imgsz=IMG_SIZE,    # <--- фиксированный размер инференса
    )

    # Прогрессбар
    pbar = tqdm(total=total_frames if total_frames > 0 else None, desc="Обработка видео")
    ...
    for result in tracker:
        ...
        if boxes is not None and boxes.id is not None:
            ...
            for i in range(len(ids)):
                ...
                x1, y1, x2, y2 = xyxy[i]
                cx = (x1 + x2) / 2.0
                cy = (y1 + y2) / 2.0  # опорная точка для зоны — центр бокса
                ...
                # Обновим класс (на случай дрейфа)
                st["class_id"] = cls_id
                ...
                if not st["active"] and st["seen_consec"] >= INIT_CONSEC_FRAMES:
                    st["active"] = True
                    # Для car — записываем «вход в кадр» сразу при активации
                ...
                # ------- Логика по классам -------
                if cls_id in PERSON_CLASSES and st["active"]:
                    # Определим зону по опорной точке
                    ...
                    if zone_id is not None and (not st["in_zone"] or st["current_zone"] != zone_id):
                        ...
                        if st["entry_confirm"] >= ENTRY_CONFIRM_FRAMES:
                            # если раньше были в другой зоне — считаем выход из неё (с фильтром по длительности)
                            if st["in_zone"] and st["current_zone"] is not None and st["entry_time"] is not None:
                                exit_frame = frame_idx
                                dwell_sec = (exit_frame - (st["entry_frame"] or exit_frame)) / max(fps, 1e-6)
                                if dwell_sec >= MIN_ZONE_DWELL_SEC:
                                    rows.append({
                                        "object_type": st["class_name"],
                                        "entry_time": st["entry_time"],
                                        "exit_time": hhmmss_from_frame(exit_frame, fps),
                            ...
                            st["entry_frame"] = frame_idx       # НОВОЕ
                            st["entry_confirm"] = 0
                    ...
                    # Если вышли из всех зон
                        ...
                        if st["exit_confirm"] >= EXIT_CONFIRM_FRAMES:
                            exit_frame = frame_idx
                            dwell_sec = (exit_frame - (st["entry_frame"] or exit_frame)) / max(fps, 1e-6)
                            if st["entry_time"] is not None and dwell_sec >= MIN_ZONE_DWELL_SEC:
                                rows.append({
                                    "object_type": st["class_name"],
                                    "entry_time": st["entry_time"],
                                    "exit_time": hhmmss_from_frame(exit_frame, fps),
        ...
        for tid, st in list(track_state.items()):
            ...
            if missed > LOST_FRAMES_TOLERANCE:
                ...
                if st["class_id"] == 3 and st["active"] and ENABLE_CAR_LOGGING:
                    ...
                    # Найти последнюю строку без exit_time для car и закрыть её по track_id
                    for r in reversed(rows):
                        if r["object_type"] == "car" and r["exit_time"] is None and r.get("track_id") == tid:
    ...
    # После прохода видео: если кто-то «завис» в зоне — закрыть по финальному кадру (с фильтром по длительности)
    ...
    # --- Сохранение CSV ---
    # Требуемые столбцы: object_type, entry_time, exit_time, zone, track_id
    # Для car zone всегда "None" (строкой).
    for r in rows:
        if r["object_type"] == "car":
            r["zone"] = "None"

    df = pd.DataFrame(rows, columns=["object_type", "entry_time", "exit_time", "zone", "track_id"])
