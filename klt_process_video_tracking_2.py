...
# Текущие рабочие гиперы selector-а.
# Важно: torso_y_min/max считаются от bbox predicted mask,
# если у тебя обновлённый model_and_selector.py.
...
# Главная логика точек
MIN_STABLE_POINTS_TO_CONTINUE = 3
REFILL_POINTS_IF_BELOW = 4       # то есть добираем только когда осталось ровно 3
...
class Track:
    track_id: int
    bbox: np.ndarray  # xyxy, original detector bbox, not expanded
...
def refresh_tracks_with_yolo(
    ...
    for ti, di, score in matches:
        ...
        if len(tr.points) < MIN_STABLE_POINTS_TO_CONTINUE:
            tr.needs_yolo = True
...
def refill_points_if_needed(
    frame_bgr: np.ndarray,
    track: Track,
    student_model: MicroNeXtMaskNet,
):
    """
    Не заменяет живые точки.
    Добирает недостающие только если осталось ровно 3 точки.
    4/5 — допустимый режим, ничего не делаем.
    """
    if track.points is None:
        track.points = np.zeros((0, 2), dtype=np.float32)

    if len(track.points) >= REFILL_POINTS_IF_BELOW:
        return

    if len(track.points) < MIN_STABLE_POINTS_TO_CONTINUE:
        track.needs_yolo = True
        track.status = "too_few_points_for_refill"
        return

    candidates = predict_points_for_bbox(frame_bgr, track.bbox, student_model)

    if not candidates:
        track.status = "refill_no_candidates"
        track.confidence = min(track.confidence, len(track.points) / TARGET_POINTS)
        return
    ...
    if len(track.points) > old_count:
        track.status = f"refilled_{old_count}_to_{len(track.points)}"
    else:
        track.status = f"refill_failed_keep_{old_count}"
...
def klt_update_track(
    ...
    if track.points is None or len(track.points) < MIN_STABLE_POINTS_TO_CONTINUE:
        track.needs_yolo = True
    ...
    if good.sum() < MIN_STABLE_POINTS_TO_CONTINUE:
    ...
    if inliers.sum() < MIN_STABLE_POINTS_TO_CONTINUE:
...
def main():
    ...
    while True:
        ...
        # Даже на scheduled YOLO кадре сначала двигаем bbox через KLT,
        # чтобы YOLO association сравнивался с текущим predicted bbox.
        ...
        if run_yolo_now:
            ...
            # Никакого полного refresh точек.
            # 4/5 и 5/5 живых точек — ничего не делаем.
            # Ровно 3 точки — пробуем добрать.
            # <3 уже должно было выставить needs_yolo внутри klt_update_track.
            for tr in tracks:
                if tr.needs_yolo:
                    continue
                ...
                if len(tr.points) < MIN_STABLE_POINTS_TO_CONTINUE:
                    tr.needs_yolo = True
                    tr.status = "too_few_stable_points"
                    continue

                if len(tr.points) < REFILL_POINTS_IF_BELOW:
                    refill_points_if_needed(frame_bgr, tr, student_model)
